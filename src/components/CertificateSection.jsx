...
const CertificateSection = ({ userData }) => {
    ...
    const checkCertificateStatus = async () => {
      try {
        ...
        if (response.data.success) {
          setCertificateStatus(response.data.certificateStatus)
          console.log("Certificate status:", response.data.certificateStatus)
        }
      } catch (error) {
  ...
  const handleAccessCertificate = async () => {
    ...
    if (!certificateStatus?.hasCertificateLink) {
      setMessage("Aucun lien de certificat disponible")
      return
    }
    ...
    try {
      console.log("Requesting certificate download link...")

      // Get the download link
      const response = await api.post("/certificate/download")

      if (response.data.success) {
        const { downloadUrl, viewUrl, fileName } = response.data.data || response.data
        ...
        // Open Google Drive link in new tab (use viewUrl for viewing, downloadUrl for direct download)
        const linkToOpen = viewUrl || downloadUrl
        ...
        try {
          const token = localStorage.getItem("token")
          await api.post("/certificate/mark-downloaded", { studentId: userData._id })
        } catch (markError) {
          console.error("Error marking certificate as downloaded:", markError)
          // Don't show error to user as the main action succeeded
        }

        // Optional: Provide direct download option
        if (downloadUrl && downloadUrl !== linkToOpen) {
          setTimeout(() => {
  ...
  return (
    <div className="card p-30 text-center">
        ...
        <div style={{ textAlign: "left" }}>
          ...
          {isEligible && certificateStatus && (
            ...
            >
              <span
                style={{
                  color: certificateStatus.hasCertificateLink ? "#2e7d32" : "#d32f2f",
                  fontSize: "1.2rem",
                }}
              >
                {certificateStatus.hasCertificateLink ? "✓" : "✗"}
              </span>
      ...
      {/* Google Drive Info */}
      {isEligible && certificateStatus?.hasCertificateLink && (
        <div
          style={{
            marginBottom: "20px",
            padding: "15px",
      ...
      <button
        ...
        disabled={!isEligible || !certificateStatus?.hasCertificateLink || loading}
        className={`btn ${isEligible && certificateStatus?.hasCertificateLink ? "btn-success" : "btn-danger"}`}
        style={{
          width: "100%",
          opacity: !isEligible || !certificateStatus?.hasCertificateLink ? 0.6 : 1,
          cursor: !isEligible || !certificateStatus?.hasCertificateLink ? "not-allowed" : "pointer",
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
          gap: "10px",
